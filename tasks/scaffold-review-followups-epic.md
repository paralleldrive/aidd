...
**Requirements**:
- Given `~/.aidd/scaffold/` already exists when `downloadExtension` is called, should throw `ScaffoldValidationError` with a message that names the path and instructs the user to run `npx aidd scaffold-cleanup`
