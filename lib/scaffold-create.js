 ...
 *
 * Architecture: resolveExtension returns { manifestPath, readmePath, downloaded }.
 * runCreate then copies path.dirname(manifestPath) → folder via fs.copy for ALL
 * scaffold sources (named, file://, and remote URL).  Named and file:// scaffolds
 * set downloaded:false so no cleanup runs; remote URL scaffolds set downloaded:true
 * so ~/.aidd/scaffold/ is removed in the finally block.
 *
...
const runCreate = async ({
  ...
  agentConfig,
  prompt,
  // __dirname is the lib/ directory of the installed aidd package.  resolveNamed
  // uses path.resolve(packageRoot, '../ai/scaffolds') so this correctly points to
  // the bundled scaffolds whether the CLI runs from the npm cache (npx aidd) or
  // from a local workspace (node bin/aidd.js).  No local clone is needed.
