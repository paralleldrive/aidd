...
describe("executeClone error result shape", () => {
  ...
  test("returns failure with a descriptive error message when clone fails", async () => {
    /** @type {any} */
    const result = await executeClone({ targetDirectory: tempDir });

    assert({
      given: "a clone that fails because ai/ already exists",
      should:
        "return success:false with a message indicating --force is needed",
      actual: {
