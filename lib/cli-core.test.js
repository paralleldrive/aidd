...
describe("executeClone error result shape", () => {
  ...
  test("returns a CausedError when clone fails", async () => {
    /** @type {any} */
    const result = await executeClone({ targetDirectory: tempDir });

    assert({
      given: "a clone that fails validation",
      should: "return success:false with an Error that has a structured cause",
      actual: {
        success: result.success,
        isCausedError:
          result.error instanceof Error &&
          !!(/** @type {any} */ (result.error).cause),
      },
      expected: { success: false, isCausedError: true },
    });
  });
});
