...
describe("executeClone dry-run config.yml status", () => {
  ...
  test("AGENTS.md does not exist", async () => {
    await executeClone({ targetDirectory: tempDir, dryRun: true });

    const logMessages = consoleSpy.mock.calls.map((call) => String(call[0]));
    const agentsMdLine = logMessages.find((msg) => msg.includes("AGENTS.md"));

    assert({
      given: "a project without an existing aidd-custom/AGENTS.md",
      should: "show (new) status in dry-run output",
      actual: agentsMdLine?.includes("(new)"),
      expected: true,
    });
  });

  test("AGENTS.md already exists", async () => {
    await fs.ensureDir(path.join(tempDir, "aidd-custom"));
    await fs.writeFile(
    ...
    assert({
      given: "a project with an existing aidd-custom/AGENTS.md",
      should: "show (exists (skipped)) status in dry-run output",
      actual: agentsMdLine?.includes("(exists (skipped))"),
      expected: true,
    });
  });
});
