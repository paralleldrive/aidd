...
import { createAiddCustomConfig, createLogger, resolvePaths } from "./cli-core.js";
...
describe("createAiddCustomConfig", () => {
  ...
  test("skips creation when config.yml already exists without force", async () => {
    ...
    assert({
      given: "an existing config.yml and no --force flag",
  ...
  test("overwrites config.yml when force is true", async () => {
    await fs.ensureDir(path.join(tempDir, "aidd-custom"));
    await fs.writeFile(
      path.join(tempDir, "aidd-custom", "config.yml"),
      "e2eBeforeCommit: true\n",
    ...
    const result = await createAiddCustomConfig({
      targetBase: tempDir,
      force: true,
    })();

    assert({
      given: "an existing config.yml with --force",
      should: "return created: true",
      actual: result.created,
      expected: true,
    });
    ...
    assert({
      given: "an existing config.yml with --force",
      should: "overwrite with default content",
      actual: content.includes("e2eBeforeCommit: false"),
      expected: true,
    });
  });
