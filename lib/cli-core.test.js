...
describe("executeClone error result shape", () => {
  ...
  test("returns an Error instance with ValidationError cause when clone fails", async () => {
    /** @type {any} */
    const result = await executeClone({ targetDirectory: tempDir });

    assert({
      given: "a clone that fails validation",
      should: "return an Error instance with a ValidationError cause code",
      actual: {
        isError: result.error instanceof Error,
        causeCode: result.error?.cause?.code,
      },
      expected: {
        isError: true,
        causeCode: "VALIDATION_ERROR",
      },
    });
  });
});
