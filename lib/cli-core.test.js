...
import { resolvePaths, createLogger } from "./cli-core.js";
...
describe("resolvePaths", () => {
  ...
  test("ai folder target path", () => {
    ...
    // Cross-platform path comparison: normalize expected path with platform-specific separator
    const expectedSuffix = path.join("my-project", "ai");

    assert({
      given: "a target directory",
      should: "create ai subfolder within target",
      actual: paths.target.endsWith(expectedSuffix),
      expected: true,
    });
  });
});
...
describe("createLogger", () => {
  ...
  test("verbose logging flag", () => {
    ...
    const defaultLogger = createLogger({ verbose: false });
