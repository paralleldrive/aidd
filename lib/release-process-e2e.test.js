...
describe("Enhanced Release Process E2E", () => {
  ...
  test("hook integration with stable version", async () => {
    ...
    // Helper to retry git operations with exponential backoff
      ...
      for (let i = 0; i < maxRetries; i++) {
        try {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, 50 * Math.pow(2, i)),
          );
        }
      }
    };
    ...
    // Verify tag exists with retry before proceeding
    await retryGitOperation(() => execAsync(`git rev-parse ${testVersion}`));
    ...
    // Verify latest tag was created and points to the test version with retry
    const { stdout: latestRef } = await retryGitOperation(() =>
      execAsync("git rev-parse latest"),
    );
    const { stdout: testRef } = await retryGitOperation(() =>
      execAsync(`git rev-parse ${testVersion}`),
    );
