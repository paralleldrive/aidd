...
// Match only genuine aidd CLI invocations:
//   - via a package runner: `npx aidd`, `bunx aidd`, `yarn dlx aidd`, `pnpm dlx aidd`
//   - flags (e.g. `-y`, `--yes`) between the runner name and `aidd` are allowed
//   - or as the first token of the command string: `aidd .`
// Rejects false positives like `echo aidd` (argument) or `npm install aidd-something`
// (hyphenated package name).
const AIDD_INVOCATION_RE =
  /(?:(?:^|\s)(?:npx|bunx|yarn(?:\s+dlx)?|pnpm(?:\s+dlx)?)\s+(?:-\S+\s+)*aidd|^aidd)(?![-\w])/;
...
const parseManifest = (content) => {
  ...
  const firstAiddRunIndex = steps.findIndex(
    (s) => s.run !== undefined && AIDD_INVOCATION_RE.test(s.run),
  );
...
/**
 * @param {{ manifestPath: string, folder: string, agentConfig?: string | { command: string, args?: string[] } | undefined, execStep?: typeof defaultExecStep }} options
 */
const runManifest = async ({
  ...
  for (const step of steps) {
    if (step.run !== undefined) {
      ...
      await runAgent({
        // agentCfg is defined: hasPromptStep was true before the loop (so config was resolved), and this branch runs only when step.prompt !== undefined.
