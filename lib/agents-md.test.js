  ...
  describe("hasAllDirectives", () => {
    ...
    test("is case insensitive", () => {
      const mixedCaseContent = `
        ...
        IMPORT AIDD-CUSTOM/AGENTS.MD // override
...
`;
  ...
  describe("requiredDirectives", () => {
    test("ensures agents can discover all essential project resources", () => {
      assert({
        given: "agents need to navigate the ai/ directory structure",
      ...
      assert({
        given: "agents must align work with project vision",
      ...
      assert({
        given: "agents need to discover project-specific customizations",
      ...
      assert({
        given: "users need to override agent instructions per project",
        should: "include import directive for aidd-custom/AGENTS.md",
        actual: requiredDirectives.includes("import aidd-custom/AGENTS.md"),
        expected: true,
      });

      assert({
        given: "agents need a quick-reference task index for common operations",
        should: "include Task Index directive",
        actual: requiredDirectives.includes("Task Index"),
        expected: true,
      });
    });
  });
  ...
  describe("directiveAppendSections", () => {
    test("provides granular upgrade support for missing directives", () => {
      assert({
        given: "users upgrading from older versions",
        should: "have directive sections available as structured data",
        actual: Array.isArray(directiveAppendSections),
        expected: true,
      });

      assert({
        given: "upgrading users need only missing directives appended",
        should: "have mappable content and keyword selectors for each section",
      ...
      assert({
        given: "upgrading users missing the Task Index",
        should: "have a Task Index section in directiveAppendSections",
        actual: directiveAppendSections.some((s) =>
          s.keywords.includes("Task Index"),
        ),
        expected: true,
      });
    });
  ...
  describe("agentsMdContent", () => {
    test("includes the Task Index section", () => {
      assert({
        given: "fresh install template",
        should: "include the Task Index section",
        actual: agentsMdContent.includes("## Task Index"),
        expected: true,
      });

      assert({
        given: "fresh install template",
        should: "include the fix bug task index entry",
        actual: agentsMdContent.includes("fix bug => /aidd-fix"),
        expected: true,
      });

      assert({
        given: "fresh install template",
        should: "include the review pull request task index entry",
        actual: agentsMdContent.includes("review pull request => /aidd-pr"),
        expected: true,
      });
    });
  });

  describe("agentsMdContent formatting", () => {
    test("import directive should have no leading whitespace", async () => {
      const rootAgentsMd = await fs.readFile(
        path.join(process.cwd(), "AGENTS.md"),
        "utf-8",
      );
      const rootImportLine = rootAgentsMd
        .split("\n")
        .find((line) => line.includes("import aidd-custom/AGENTS.md"));

      const templateImportLine = agentsMdContent
        .split("\n")
        .find((line) => line.includes("import aidd-custom/AGENTS.md"));

      assert({
        given: "root AGENTS.md has an import directive",
        should: "be found in the file",
        actual: rootImportLine !== undefined,
        expected: true,
      });

      assert({
        given: "agentsMdContent template has an import directive",
        should: "be found in the template",
        actual: templateImportLine !== undefined,
        expected: true,
      });

      assert({
        given: "agentsMdContent template import directive",
        should: "exactly match the root AGENTS.md import directive format",
        actual: templateImportLine,
        expected: rootImportLine,
      });

      assert({
        given: "agentsMdContent template import directive",
        should: "start with 'import' (no leading space)",
        actual: templateImportLine?.startsWith("import"),
        expected: true,
      });
    });
  });
