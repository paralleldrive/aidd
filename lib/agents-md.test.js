  ...
  describe("agentsMdContent e2e instruction", () => {
    ...
    test("explains that E2E tests are excluded from the pre-commit hook", () => {
      assert({
        ...
        actual:
          agentsMdContent.includes("pre-commit") ||
          agentsMdContent.includes("commit hook"),
        expected: true,
      });
    });
  });
...
describe("syncRootAgentFiles", () => {
  ...
  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `aidd-sync-${Date.now()}`);
    await fs.ensureDir(tempDir);
  });
  ...
  test("creates both files when neither exists", async () => {
    ...
    assert({
      given: "a directory with no AGENTS.md or CLAUDE.md",
      should: "return created action for AGENTS.md",
      actual: results.find((r) => r.file === "AGENTS.md")?.action,
      expected: "created",
    });

    assert({
      given: "a directory with no AGENTS.md or CLAUDE.md",
      should: "return created action for CLAUDE.md",
      actual: results.find((r) => r.file === "CLAUDE.md")?.action,
      expected: "created",
    });
    ...
    assert({
      given: "newly created AGENTS.md",
      should: "contain agentsMdContent exactly",
      actual: agentsContent,
  ...
  test("returns unchanged when both files already match the template", async () => {
    ...
    await fs.writeFile(path.join(tempDir, "CLAUDE.md"), agentsMdContent);

    const results = await syncRootAgentFiles(tempDir);

    assert({
      given: "AGENTS.md already matching agentsMdContent",
      should: "return unchanged for AGENTS.md",
      actual: results.find((r) => r.file === "AGENTS.md")?.action,
      expected: "unchanged",
    });
    ...
    assert({
      ...
      should: "return unchanged for CLAUDE.md",
      actual: results.find((r) => r.file === "CLAUDE.md")?.action,
      expected: "unchanged",
    });
  });
  ...
  test("overwrites a stale file whose content differs from the current template", async () => {
    ...
    await fs.writeFile(path.join(tempDir, "AGENTS.md"), staleContent);
    await fs.writeFile(path.join(tempDir, "CLAUDE.md"), agentsMdContent);

    const results = await syncRootAgentFiles(tempDir);

    assert({
      given: "AGENTS.md with stale content differing from agentsMdContent",
      should: "return updated action for AGENTS.md",
      actual: results.find((r) => r.file === "AGENTS.md")?.action,
      expected: "updated",
    });
    ...
    assert({
      given: "AGENTS.md after sync",
      should: "contain the current agentsMdContent exactly",
      actual: agentsContent,
      expected: agentsMdContent,
    });
  });
...
describe("ensureClaudeMd", () => {
  ...
  test("throws AgentsFileError when CLAUDE.md exists but cannot be read", async () => {
    await fs.writeFile(path.join(tempDir, "CLAUDE.md"), "# existing content");

    // Simulate an I/O read failure
    const ioError = Object.assign(new Error("EACCES: permission denied"), {
      code: "EACCES",
    });
    const spy = vi.spyOn(fs, "readFile").mockRejectedValueOnce(ioError);

    /** @type {any} */
    let error = null;
    try {
      await ensureClaudeMd(tempDir);
    } catch (err) {
      error = err;
    } finally {
      spy.mockRestore();
    ...
    assert({
      given: "CLAUDE.md exists but readFile throws a permission error",
      should: "throw AgentsFileError wrapping the original error",
      actual: error?.cause?.code,
      expected: "AGENTS_FILE_ERROR",
    });
  });
  ...
  test("leaves CLAUDE.md unchanged when created by a previous install", async () => {
    // First install creates CLAUDE.md with agentsMdContent, which has all
    // required directives but no literal "AGENTS.md" substring — the previous
    // existingContent.includes("AGENTS.md") check returned false here,
    // causing the pointer line to be appended on every second install.
    await fs.writeFile(path.join(tempDir, "CLAUDE.md"), agentsMdContent);
    ...
    assert({
      given:
        "CLAUDE.md created by a previous install containing all agent directives",
      should: "return unchanged without modifying the file",
      actual: result.action,
      expected: "unchanged",
    });
    ...
    assert({
      given: "CLAUDE.md created by a previous install",
      should: "preserve the original content exactly",
      actual: content,
      expected: agentsMdContent,
    });
