...
// Clones the entire repo so scaffolds can carry arbitrary files, templates,
// package.json dependencies, config files, etc.
const defaultGitClone = (repoUrl, destPath) =>
...
const cloneExtension = async ({ uri, folder, clone }) => {
  const scaffoldDir = path.join(folder, ".aidd/scaffold");
  // Remove any prior clone so we start clean
  await fs.remove(scaffoldDir);
  await clone(uri, scaffoldDir);
  return {
...
const resolveExtension = async ({
  ...
  if (isHttpUrl(effectiveType)) {
    ...
    try {
      paths = await cloneExtension({ clone, folder, uri: effectiveType });
    } catch (originalError) {
      throw createError({
        ...ScaffoldNetworkError,
        cause: originalError,
        message: `Failed to clone scaffold from ${effectiveType}: ${originalError.message}`,
      });
    }
