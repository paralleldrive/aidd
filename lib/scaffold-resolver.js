...
const resolveNamed = ({ type, packageRoot }) => {
  ...
  if (!typeDir.startsWith(scaffoldsRoot + path.sep)) {
    throw createError({
      ...ScaffoldValidationError,
      message: `Invalid scaffold type "${type}": must be a simple name, not a path. Resolved outside the scaffolds directory.`,
    });
  }
...
const resolveExtension = async ({
  ...
  const effectiveType =
    type || process.env.AIDD_CUSTOM_CREATE_URI || DEFAULT_SCAFFOLD_TYPE;
