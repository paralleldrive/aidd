...
import { getGitHubToken } from "./gh-auth.js";
import {
...
// Authentication is resolved automatically: GITHUB_TOKEN env var is checked
// first, then `gh auth token` (GitHub CLI credential store). This means users
// who have run `gh auth login` can access private repos they've been granted
// access to — even repos they don't own — without manually creating a PAT.
const defaultResolveRelease = async (
  ...
  if (!response.ok) {
    if (response.status === 403) {
      throw createError({
        ...ScaffoldNetworkError,
        message: `GitHub API rate limit exceeded for ${repoUrl}. Set GITHUB_TOKEN or run \`gh auth login\` for 5,000 req/hr.`,
      });
    }
...
// The resolved GitHub token is forwarded only for GitHub hostnames to avoid
// leaking credentials to third-party servers that happen to host a scaffold tarball.
