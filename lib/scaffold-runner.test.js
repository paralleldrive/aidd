...
describe("runManifest", () => {
  ...
  test("executes prompt steps via agent CLI", async () => {
    ...
    assert({
      ...
      should: "call runAgent with the correct agentConfig, cwd, and prompt",
      actual: vi.mocked(runAgent).mock.calls[0][0],
      expected: {
  ...
  test("propagates errors thrown by runAgent for prompt steps", async () => {
    const manifestPath = path.join(tempDir, "SCAFFOLD-MANIFEST.yml");
    await fs.writeFile(
      manifestPath,
      "steps:\n  - run: npx aidd .\n  - prompt: do something\n",
    ...
    vi.mocked(runAgent).mockRejectedValueOnce(agentError);
    ...
    assert({
      given: "runAgent throws an error for a prompt step",
      should: "propagate the error from runManifest",
      actual: error,
      expected: agentError,
    });
  });
  ...
  test("resolves agentConfig once and passes the same config object to all prompt steps", async () => {
    const mockExecStep = async () => {};
    ...
    assert({
      ...
      should:
        "pass the same agentConfig object reference to every runAgent call",
      actual: calls[0][0].agentConfig === calls[1][0].agentConfig,
      expected: true,
    });
  });

  test("defaults to claude when agentConfig is not provided and no env var or config file is present", async () => {
    const mockExecStep = async () => {};
    ...
    assert({
      ...
      should:
        "call runAgent with the claude preset agentConfig (default when no env var or config file present)",
      actual: vi.mocked(runAgent).mock.calls[0][0].agentConfig,
      expected: { command: "claude", args: ["-p"] },
    });
  });

  test("does not call runAgent when there are no prompt steps", async () => {
    const mockExecStep = async () => {};
    ...
    assert({
      given: "a manifest with only run steps and no prompt steps",
      should: "never call runAgent",
      actual: vi.mocked(runAgent).mock.calls.length,
      expected: 0,
    });
  });
});
