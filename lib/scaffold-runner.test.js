...
describe("runManifest", () => {
  ...
  test("throws ScaffoldStepError when a prompt step spawn fails with E2BIG", async () => {
    ...
    // First spawn call is for the aidd run step — let it succeed
    vi.mocked(spawn).mockImplementationOnce(() => {
      const proc = new EventEmitter();
    ...
    // Second spawn call is for the prompt step — simulate E2BIG
    vi.mocked(spawn).mockImplementationOnce(() => {
      const proc = new EventEmitter();
      process.nextTick(() => {
        proc.emit(
    ...
    assert({
      ...
      expected: {
        code: "SCAFFOLD_STEP_ERROR",
        message: "Argument list too long for spawn: claude set up the project",
      },
    });
  });
