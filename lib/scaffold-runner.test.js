...
describe("runManifest", () => {
  ...
  test("executes prompt steps via agent CLI", async () => {
    ...
    assert({
      ...
      should: "invoke agent CLI with prompt text as a separate argument",
      actual: executed[0].command,
      expected: ["claude", "Set up the project"],
    });
  });
  ...
  test("throws ScaffoldStepError when prompt text causes E2BIG spawn failure", async () => {
    const promptText = "x".repeat(200);
    const manifestPath = path.join(tempDir, "SCAFFOLD-MANIFEST.yml");
    await fs.writeFile(manifestPath, `steps:\n  - prompt: ${promptText}\n`);

    const mockExecStep = async () => {
      const err = new Error("spawn E2BIG");
      err.code = "E2BIG";
      throw err;
    };
    ...
    assert({
      ...
      should:
        "throw ScaffoldStepError with a message including the prompt length",
      actual: {
        code: error?.cause?.code,
        includesLength:
          typeof error?.message === "string" &&
          error.message.includes(`${promptText.length} chars`),
      },
      expected: { code: "SCAFFOLD_STEP_ERROR", includesLength: true },
    });
  });
