...
Read index.md files (auto-generated from frontmatter).
...
describe("full installer workflow integration", () => {
  ...
  test("installer creates aidd-custom/index.md on fresh install", async () => {
    await executeClone({
      targetDirectory: tempTestDir,
    });

    const indexExists = await fs.pathExists(
      path.join(tempTestDir, "aidd-custom", "index.md"),
    );

    assert({
      given: "fresh installation",
      should: "create aidd-custom/index.md so agents can read it immediately",
      actual: indexExists,
      expected: true,
    });
  });

  test("installer creates aidd-custom/AGENTS.md on fresh install", async () => {
    await executeClone({
      targetDirectory: tempTestDir,
    });

    const agentsMdExists = await fs.pathExists(
      path.join(tempTestDir, "aidd-custom", "AGENTS.md"),
    );

    assert({
      given: "fresh installation",
      should:
        "create aidd-custom/AGENTS.md so users can override root AGENTS.md settings",
      actual: agentsMdExists,
      expected: true,
    });
  });
});
