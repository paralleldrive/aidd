...
`,
  ...
  test("--index generates index.md files", async () => {
    ...
    // Note: ai/commands/index.md is NOT created by --index
    // It's managed by --skills (skills-extractor) to avoid conflicts
  });
  ...
  test("generated index includes file descriptions from frontmatter", async () => {
    ...
    assert({
      given: "generated rules index",
      should: "include file description from frontmatter",
      actual: rulesIndex.includes("Test rule for TDD"),
      expected: true,
    });
  });
