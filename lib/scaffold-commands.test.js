...
describe("registerScaffoldCommands", () => {
  ...
  test("registers the create command with an --agent option", () => {
    const program = createTestProgram();
    registerScaffoldCommands(program);
    ...
    assert({
      ...
      should: "have an --agent option",
      actual: agentOption?.long,
      expected: "--agent",
    });
  });

  test("registers the create command with a default agent of 'claude'", () => {
    const program = createTestProgram();
    registerScaffoldCommands(program);
    ...
    assert({
      given: "the create command --agent option",
