...
describe("registerScaffoldCommands", () => {
  ...
  test("registers the expected command names on the program", () => {
    ...
    assert({
      ...
      should:
        "register the create, verify-scaffold, and set commands (scaffold-cleanup is internal, not public)",
      actual: commandNames(program),
      expected: ["create", "set", "verify-scaffold"],
    });
  });

  test("registers the create command with an --agent-config option", () => {
    const program = createTestProgram();
    registerScaffoldCommands(program);

    const createCmd = program.commands.find((cmd) => cmd.name() === "create");
    const agentOption = createCmd?.options.find(
      (opt) => opt.long === "--agent-config",
    );

    assert({
      given: "the create command",
      should: "have an --agent-config option",
      actual: agentOption?.long,
      expected: "--agent-config",
    });
  });

  test("registers the create command --agent-config option with no default so the resolution chain runs", () => {
    const program = createTestProgram();
    registerScaffoldCommands(program);
    ...
    assert({
      given:
        "the create command --agent-config option is not explicitly passed",
      should:
