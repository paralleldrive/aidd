...
`,
    ...
    .option(
      ...
      "agent name or config YAML path for prompt steps",
      "claude",
    )
    .option(
      "--prompt <text>",
      "Run an agent prompt in the new project after scaffolding",
    )
    // @ts-expect-error -- Commander action callback parameters are untyped
    .action(async (typeOrFolder, folder, { agentConfig, prompt }) => {
