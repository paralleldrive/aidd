...
describe("resolveAgentConfig", () => {
  ...
  test("plain agent name string 'cursor' returns cursor preset", async () => {
    assert({
      given: "agent name string 'cursor'",
      should: "return { command: 'cursor', args: ['agent', '--print'] }",
      actual: await resolveAgentConfig({ value: "cursor" }),
      expected: { command: "cursor", args: ["agent", "--print"] },
    });
  });
  ...
  test("numeric value rejects with AgentConfigValidationError (same path as resolveByKind non-string primitive)", async () => {
    let error;
    try {
      await resolveAgentConfig({ value: /** @type {any} */ (42) });
    } catch (e) {
      error = /** @type {any} */ (e);
    }
    ...
    assert({
      given:
        "value is a non-string primitive (42), same resolution as resolveByKind(42, (p) => p)",
      should: "reject with AgentConfigValidationError, not TypeError",
      actual: error?.cause?.name,
      expected: "AgentConfigValidationError",
    });
  });
  ...
  test("inline object in aidd-custom/config.yml missing command warns and falls back to claude", async () => {
    const tempDir = await createTempDir();
    const aiddCustomDir = path.join(tempDir, "aidd-custom");
    await fs.ensureDir(aiddCustomDir);
    await fs.writeFile(
      path.join(aiddCustomDir, "config.yml"),
      yaml.dump({ "agent-config": { args: ["--run"] } }),
      "utf-8",
    );
    ...
    assert({
      given:
        "an inline YAML object in aidd-custom/config.yml missing the command field",
      should: "warn with a helpful message",
      actual: warnSpy.mock.calls.length > 0,
      expected: true,
    });
  ...
  test("agent-config: false in aidd-custom/config.yml warns and falls back to claude", async () => {
    const tempDir = await createTempDir();
    const aiddCustomDir = path.join(tempDir, "aidd-custom");
    await fs.ensureDir(aiddCustomDir);
    await fs.writeFile(
    ...
    assert({
      given: "agent-config: false in aidd-custom/config.yml",
      should: "warn rather than silently ignoring the value",
      actual: warnSpy.mock.calls.length > 0,
      expected: true,
    });
  ...
  test("agent-config: 42 (a number) in aidd-custom/config.yml warns with validation error and falls back to claude", async () => {
    const tempDir = await createTempDir();
    const aiddCustomDir = path.join(tempDir, "aidd-custom");
    await fs.ensureDir(aiddCustomDir);
    await fs.writeFile(
    ...
    const result2 = await resolveAgentConfig({ cwd: tempDir });

    assert({
      given: "agent-config: 42 (a number) in aidd-custom/config.yml",
      should:
        "warn with an AgentConfigValidationError message, not a TypeError",
      actual: warnSpy.mock.calls[0]?.[0]?.includes("Invalid agent config"),
      expected: true,
    });
