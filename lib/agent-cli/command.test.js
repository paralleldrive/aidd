...
describe("registerAgentCommand", () => {
  ...
  test("passes --agent-config value to resolveAgentConfig", async () => {
    ...
    assert({
      given: "--agent-config flag set to 'opencode'",
  ...
  test("prints an error and exits 1 when --prompt is not provided", async () => {
    ...
    assert({
      ...
      expected: chalk.red("error: missing required option '--prompt <text>'"),
    });
  ...
  test.each([
    ["AgentConfigReadError", AgentConfigReadError],
    ["AgentConfigParseError", AgentConfigParseError],
    ["AgentConfigValidationError", AgentConfigValidationError],
  ])("prints formatted error message and exits 1 on %s", async (errorTypeName, errorType) => {
    vi.mocked(resolveAgentConfig).mockRejectedValueOnce(createError(errorType));
    ...
    assert({
      given: `resolveAgentConfig throws ${errorTypeName}`,
      should: "print the formatted config error message to stderr",
      actual: consoleErrorSpy.mock.calls[0]?.[0],
      expected: chalk.red(`\n❌ Agent config error: ${errorType.message}`),
    });
