...
describe("runAgent", () => {
  ...
  test("rejects with ScaffoldStepError when prompt is an empty string", async () => {
    let error = null;
    try {
      await runAgent({
        agentConfig: { command: "claude", args: ["-p"] },
    ...
    assert({
      given: "prompt is an empty string",
      should:
        "reject with ScaffoldStepError without spawning the agent process",
      actual: /** @type {any} */ (error)?.cause?.code,
      expected: "SCAFFOLD_STEP_ERROR",
    });
    ...
    assert({
      ...
      actual: vi.mocked(spawn).mock.calls.length,
      expected: 0,
    });
  });
});
