...
const agentPresets = {
  ...
  cursor: { args: ["agent", "--print"], command: "cursor" },
...
const loadYamlConfig = async (filePath) => {
  ...
  if (
    ...
    throw createError({
      ...AgentConfigValidationError,
      message: "Agent config must contain a 'command' field",
    });
  }
...
const valueKindHandlers = {
  inline: async (v) => {
    ...
    if (!("command" in obj) || !obj.command) {
      throw createError({
        ...AgentConfigValidationError,
        message: "Agent config must contain a 'command' field",
      });
    }
    return /** @type {{ command: string, args?: string[] }} */ (v);
  },
  invalid: async () => {
    throw createError({
      ...AgentConfigValidationError,
  ...
  preset: async (v) => getAgentConfig(/** @type {string} */ (v)),
  yaml: (v, rp) => loadYamlConfig(rp(/** @type {string} */ (v))),
};

const resolveByKind = (value, resolveYamlPath) => {
  const kind =
    // null/arrays are typeof "object" and would wrongly become "inline" without this guard
    value === null || Array.isArray(value)
      ...
      : isYamlPath(value)
        ...
        : typeof value === "object"
          ? "inline"
          : // boolean, number, undefined, symbol, bigint (not caught above)
            typeof value !== "string"
            ? "invalid"
            : "preset";
