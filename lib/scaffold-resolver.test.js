...
/** Deterministic "no credentials" for tests (avoids flaky `gh auth token` on dev machines). */
const noGithubAuth = async () => undefined;

/** Env-only token resolution — models GITHUB_TOKEN / GH_TOKEN fallback without GitHub CLI. */
const authTokenFromEnvOnly = async () =>
...
describe("defaultResolveRelease - GITHUB_TOKEN auth and error messages", () => {
  ...
  test("includes Authorization header when getAuthToken returns a GitHub CLI session token", async () => {
    ...
    await defaultResolveRelease("https://github.com/org/repo", {
      getAuthToken: async () => "gh-cli-session-token",
    });

    assert({
      given: "getAuthToken yields a token (e.g. from gh auth login)",
      should: "attach Authorization Bearer for the GitHub API request",
      actual: capturedHeaders.Authorization,
      expected: "Bearer gh-cli-session-token",
    });
  });
  ...
  test("includes Authorization header when GITHUB_TOKEN is set", async () => {
    ...
    await defaultResolveRelease("https://github.com/org/repo", {
      getAuthToken: authTokenFromEnvOnly,
    });
  ...
  test("error for 403 mentions rate limit and authentication options", async () => {
    // @ts-expect-error
    ...
    assert({
      given: "GitHub API returns 403",
      should: "mention gh auth login and GITHUB_TOKEN in the error",
      actual:
        typeof error?.message === "string" &&
    ...
    assert({
      given: "GitHub API returns 403",
      should: "mention GH_TOKEN in the error",
      actual:
        typeof error?.message === "string" &&
  ...
  test("error for 404 without authentication hints how to authenticate", async () => {
    delete process.env.GITHUB_TOKEN;
    // @ts-expect-error
    ...
    assert({
      given:
        "GitHub API returns 404 and no auth token is available (env or gh)",
      should: "include gh auth login and GITHUB_TOKEN in the hint",
      actual:
        typeof error?.message === "string" &&
        error.message.includes("gh auth login") &&
        error.message.includes("GITHUB_TOKEN"),
  ...
  test("error for 404 with auth token set does not include private-repo hint", async () => {
    // @ts-expect-error
    globalThis.fetch = async () => ({ ok: false, status: 404 });
    ...
    assert({
      given: "GitHub API returns 404 and a bearer token is already in use",
      should:
        "not include private-repo authentication hint (repo may be missing or have no releases)",
      actual:
        typeof error?.message === "string" &&
        !error.message.includes("If the repo is private"),
      expected: true,
    });
  });
...
describe("defaultDownloadAndExtract - GITHUB_TOKEN auth", () => {
  ...
  test("uses GH_TOKEN for GitHub download when GITHUB_TOKEN is unset", async () => {
    process.env.GH_TOKEN = "gh-env-only";
    delete process.env.GITHUB_TOKEN;
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
    ...
    try {
      await defaultDownloadAndExtract(
        "https://codeload.github.com/org/repo/tar.gz/v1.0.0",
        "/tmp/aidd-test-gh-token-dest",
        { getAuthToken: authTokenFromEnvOnly },
      );
    } catch {
      // tar failure expected
    }

    assert({
      given: "only GH_TOKEN is set and URL is codeload.github.com",
      should: "send Bearer GH_TOKEN on the download request",
      actual: capturedHeaders.Authorization,
      expected: "Bearer gh-env-only",
    });
  });

  test("does not call getAuthToken for non-GitHub URLs", async () => {
    let authTokenCalled = false;
    const spyAuthToken = async () => {
    ...
    try {
      await defaultDownloadAndExtract(
        "https://example.com/scaffold.tar.gz",
        "/tmp/aidd-spy-auth-test-dest",
        { getAuthToken: spyAuthToken },
      );
    } catch {
      // tar failure expected — getAuthToken call behavior is what we're testing
    } finally {
      globalThis.fetch = savedFetch;
    }

    assert({
      given: "a third-party URL (not a GitHub hostname)",
      should: "not call getAuthToken at all",
      actual: authTokenCalled,
      expected: false,
    });
  });

  test("does not call getAuthToken for malformed URLs", async () => {
    let authTokenCalled = false;
    const spyAuthToken = async () => {
      authTokenCalled = true;
      return /** @type {string | undefined} */ (undefined);
    ...
    try {
      await defaultDownloadAndExtract(
        "not-a-valid-url",
        "/tmp/aidd-malformed-auth-test-dest",
        { getAuthToken: spyAuthToken },
      );
    } catch {
      // fetch failure expected for malformed URL — getAuthToken behavior is what we're testing
    } finally {
      globalThis.fetch = savedFetch;
    }

    assert({
      given: "a malformed URL (throws on new URL(url))",
      should: "not call getAuthToken at all",
      actual: authTokenCalled,
      expected: false,
    });
