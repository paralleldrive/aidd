...
describe("defaultResolveRelease - GITHUB_TOKEN auth and error messages", () => {
  ...
  test("does not include Authorization header when GITHUB_TOKEN is not set and gh is not authenticated", async () => {
    delete process.env.GITHUB_TOKEN;
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
    ...
    assert({
      given: "GITHUB_TOKEN is not set and gh is not authenticated",
  ...
  test("error for 404 without any auth hints to set the token or use gh auth login", async () => {
    delete process.env.GITHUB_TOKEN;
    // @ts-expect-error
    ...
    assert({
      given: "GitHub API returns 404 and no auth is available",
      should: "include hint to set GITHUB_TOKEN or run gh auth login",
      actual: {
        mentionsToken:
          typeof error?.message === "string" &&
          error.message.includes("GITHUB_TOKEN"),
      ...
      expected: { mentionsToken: true, mentionsGh: true },
    });
  });
...
describe("getGitHubToken - gh auth fallback", () => {
  ...
  test("returns GITHUB_TOKEN env var when set, without calling gh", async () => {
    ...
    assert({
      given: "GITHUB_TOKEN is set in the environment",
      should: "return the env token without invoking runGhAuthToken",
      actual: { token, ghCalled },
      expected: { token: "env-token-abc", ghCalled: false },
    });
  });

  test("returns token from gh auth token when GITHUB_TOKEN is not set", async () => {
    const token = await getGitHubToken({
      runGhAuthToken: async () => "gh-token-xyz",
    });

    assert({
      given: "GITHUB_TOKEN is not set and gh auth token succeeds",
      should: "return the token from gh",
      actual: token,
      expected: "gh-token-xyz",
    });
  });

  test("returns null when GITHUB_TOKEN is not set and gh auth token fails", async () => {
    const token = await getGitHubToken({
      runGhAuthToken: async () => {
    ...
    assert({
      given: "GITHUB_TOKEN is not set and gh auth token throws",
  ...
  test("returns null when GITHUB_TOKEN is not set and gh auth token returns empty string", async () => {
    const token = await getGitHubToken({
      runGhAuthToken: async () => "",
    });
  ...
  test("does not call gh auth token when GITHUB_TOKEN is set", async () => {
    ...
    assert({
      given: "GITHUB_TOKEN is set in the environment",
      should: "not invoke runGhAuthToken",
      actual: { token, ghCalled },
      expected: { token: "env-token-xyz", ghCalled: false },
    });
  });
});

describe("defaultResolveRelease - gh auth fallback", () => {
  /** @type {any} */
  let originalFetch;
  /** @type {string | undefined} */
  let savedToken;
  ...
  test("uses gh auth token when GITHUB_TOKEN is not set and gh is authenticated", async () => {
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
    globalThis.fetch = async (_url, opts = {}) => {
    ...
    await defaultResolveRelease("https://github.com/org/repo", {
      runGhAuthToken: async () => "gh-token-from-cli",
    });

    assert({
      given: "GITHUB_TOKEN is not set and gh auth token returns a token",
      should:
        "include Authorization: Bearer header using the gh token in the GitHub API request",
      actual: capturedHeaders.Authorization,
      expected: "Bearer gh-token-from-cli",
    });
  });

  test("error for 404 without any auth hints to set GITHUB_TOKEN or run gh auth login", async () => {
    // @ts-expect-error
    globalThis.fetch = async () => ({ ok: false, status: 404 });
    ...
    assert({
      given: "GitHub API returns 404 and no auth is available",
      should: "hint to set GITHUB_TOKEN or run gh auth login",
      actual: {
        mentionsToken:
          typeof error?.message === "string" &&
          error.message.includes("GITHUB_TOKEN"),
