...
describe("resolveExtension - default scaffold resolution", () => {
  ...
  test("uses next-shadcn when no type, no env var, and no config file", async () => {
    ...
    assert({
      given: "no type, no AIDD_CUSTOM_CREATE_URI env var, no config file",
  ...
  test("uses config file create-uri when no type and no env var", async () => {
    let tempDir;
    try {
      tempDir = path.join(
        os.tmpdir(),
        `aidd-config-resolver-test-${Date.now()}`,
      );
      await fs.ensureDir(tempDir);
      ...
      assert({
        given: "config file has create-uri and no env var or type",
        should: "use the config file URI as the extension source",
        actual: paths.readmePath,
        expected: path.join(tempDir, "README.md"),
      });
    } finally {
  ...
  test("env var takes precedence over config file create-uri", async () => {
    let envDir;
    let configDir;
    try {
      envDir = path.join(os.tmpdir(), `aidd-env-precedence-${Date.now()}`);
      configDir = path.join(os.tmpdir(), `aidd-cfg-precedence-${Date.now()}`);
      await fs.ensureDir(envDir);
      await fs.ensureDir(configDir);
      await fs.writeFile(path.join(envDir, "README.md"), "# Env Scaffold");
      await fs.writeFile(
        path.join(envDir, "SCAFFOLD-MANIFEST.yml"),
        "steps:\n",
      );
      await fs.writeFile(
      ...
      await fs.writeFile(
        path.join(configDir, "SCAFFOLD-MANIFEST.yml"),
        "steps:\n",
      );
      ...
      assert({
        given:
          "both AIDD_CUSTOM_CREATE_URI env var and config file create-uri are set",
        should: "use the env var (higher priority)",
        actual: paths.readmePath,
