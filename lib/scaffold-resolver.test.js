...
describe("resolveExtension - default scaffold resolution", () => {
  ...
  test("leaves AIDD_CUSTOM_CREATE_URI absent after restoring when it was originally unset", () => {
    assert({
      given: "AIDD_CUSTOM_CREATE_URI was not set before the previous test ran",
      should:
        "be absent (not the string 'undefined') after that test completes",
  ...
  test("leaves AIDD_CUSTOM_CREATE_URI absent after restoring when it was originally unset (user-config path)", () => {
    assert({
      given:
        "AIDD_CUSTOM_CREATE_URI was not set before the user-config resolution test ran",
      should:
        "be absent (not the string 'undefined') after that test completes",
      actual: process.env.AIDD_CUSTOM_CREATE_URI,
      expected: undefined,
