...
describe("defaultResolveRelease - auth and error messages", () => {
  /** @type {any} */
  let originalFetch;
  ...
  test("includes Authorization header when getToken resolves a token", async () => {
    ...
    assert({
      given: "getToken resolves a token",
  ...
  test("does not include Authorization header when no token is available", async () => {
    delete process.env.GITHUB_TOKEN;
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
  ...
  test("error for 403 mentions rate limit and gh auth login", async () => {
    // @ts-expect-error
    ...
    assert({
      given: "GitHub API returns 403",
      should: "mention gh auth login in the error",
      actual:
        typeof error?.message === "string" &&
  ...
  test("error for 404 without any token hints to authenticate", async () => {
    delete process.env.GITHUB_TOKEN;
    // @ts-expect-error
    ...
    assert({
      given: "GitHub API returns 404 and no token is available",
      should: "include authentication hint in the error",
      actual:
        typeof error?.message === "string" &&
        error.message.includes("gh auth login"),
      expected: true,
  ...
  test("error for 404 with a token does not include auth hint", async () => {
    ...
    assert({
      given: "GitHub API returns 404 and a token is available",
      should:
        "not include auth hint (token is set; repo simply does not exist or has no releases)",
      actual:
        typeof error?.message === "string" &&
...
describe("defaultDownloadAndExtract - auth", () => {
  /** @type {any} */
  let originalFetch;
  ...
  test("includes Authorization header for api.github.com URLs when token is available", async () => {
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
    globalThis.fetch = async (_url, opts = {}) => {
    ...
    assert({
      given: "a token is available and URL hostname is api.github.com",
  ...
  test("does not include Authorization header for third-party URLs even when token is available", async () => {
    /** @type {Record<string, string>} */
    const capturedHeaders = {};
    // @ts-expect-error
    globalThis.fetch = async (_url, opts = {}) => {
    ...
    assert({
      given: "a token is available and URL is a third-party host",
      should:
