...
describe("downloadExtension", () => {
  ...
  test("throws ScaffoldValidationError when scaffoldDownloadDir already exists", async () => {
    const scaffoldDir = path.join(tempDir, "scaffold");
    ...
    try {
      await downloadExtension({
        uri: "https://example.com/scaffold.tar.gz",
        scaffoldDownloadDir: scaffoldDir,
        download: mockDownload,
        existsFn,
      });
    ...
    assert({
      ...
      should:
        "throw ScaffoldValidationError naming the path and instructing scaffold-cleanup",
      actual: {
        code: error?.cause?.code,
        mentionsPath: error?.message?.includes(scaffoldDir),
        mentionsCleanup: error?.message?.includes("scaffold-cleanup"),
      },
      expected: {
        code: "SCAFFOLD_VALIDATION_ERROR",
        mentionsPath: true,
        mentionsCleanup: true,
      },
    });
  });
