...
describe("runCreate", () => {
  ...
  /** Returns a { fn, calls } pair for tracking scaffoldCleanupFn invocations. */
  const makeCleanupTracker = () => {
    const calls = /** @type {Array<unknown>} */ ([]);
    return {
      calls,
      fn: async (/** @type {unknown} */ args) => calls.push(args),
    };
  };
  ...
  test("calls scaffoldCleanupFn even when copyFn throws after download", async () => {
    const { calls: cleanupCalls, fn: trackingCleanup } = makeCleanupTracker();
    ...
    assert({
      given: "copyFn throws after a successful download",
      should: "still call scaffoldCleanupFn once to prevent scaffold dir leak",
  ...
  test("calls scaffoldCleanupFn even when runManifest throws (cleanup in finally)", async () => {
    const { calls: cleanupCalls, fn: trackingCleanup } = makeCleanupTracker();
    const downloadedResolve = async () => ({
      manifestPath: "/fake/SCAFFOLD-MANIFEST.yml",
      readmePath: "/fake/README.md",
    ...
    assert({
      given: "runManifest throws with a downloaded scaffold",
      should: "still call scaffoldCleanupFn once (cleanup in finally)",
      actual: cleanupCalls.length,
      expected: 1,
    });

    assert({
      given: "runManifest throws",
      should: "propagate the error to the caller",
      actual: /** @type {any} */ (error)?.message,
  ...
  test("propagates original manifest error when scaffoldCleanupFn also throws", async () => {
    const downloadedResolve = async () => ({
      manifestPath: "/fake/SCAFFOLD-MANIFEST.yml",
      readmePath: "/fake/README.md",
      downloaded: true,
    ...
    try {
      await runCreate({
        ...
        copyFn: noopCopy,
        existsFn: noopExists,
        scaffoldCleanupFn: failingCleanup,
      });
    } catch (e) {
      error = e;
    }
    ...
    assert({
      given: "scaffoldCleanupFn throws and runManifestFn also threw",
      should: "propagate the original manifest error (not the cleanup error)",
      actual: /** @type {any} */ (error)?.message,
      expected: "manifest error",
    });
  });

  test("resolves successfully when scaffoldCleanupFn throws but runManifestFn succeeded", async () => {
    const downloadedResolve = async () => ({
      manifestPath: "/fake/SCAFFOLD-MANIFEST.yml",
      readmePath: "/fake/README.md",
      downloaded: true,
    ...
    assert({
      given: "scaffoldCleanupFn throws but runManifestFn succeeded",
      should: "still resolve with success:true and the folderPath",
      actual: { success: result.success, folderPath: result.folderPath },
      expected: { success: true, folderPath: "/abs/my-project" },
    });
  });
