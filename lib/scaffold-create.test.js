...
describe("runCreate", () => {
  ...
  test("passes type and project-scoped scaffoldDownloadDir to resolveExtension", async () => {
    const calls =
      /** @type {Array<{type?: string, scaffoldDownloadDir?: string}>} */ ([]);
    const trackingResolve = async (
      /** @type {{type?: string, scaffoldDownloadDir?: string}} */ {
        type,
        scaffoldDownloadDir,
      },
    ) => {
      calls.push({ scaffoldDownloadDir, type });
      return {
        manifestPath: "/fake/SCAFFOLD-MANIFEST.yml",
        readmePath: "/fake/README.md",
      };
    ...
    assert({
      ...
      should:
        "pass type and project-scoped scaffoldDownloadDir to resolveExtension",
      actual: {
        type: calls[0]?.type,
        scaffoldDownloadDir: calls[0]?.scaffoldDownloadDir,
      },
      expected: {
        type: "scaffold-example",
        scaffoldDownloadDir: path.join("/abs/my-project", ".aidd", "scaffold"),
      },
    });
  });
