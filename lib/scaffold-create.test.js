...
describe("runCreate", () => {
  ...
  test("includes cleanupTip when scaffold was downloaded (downloaded:true)", async () => {
    const result = await runCreate({
      folder: projectDir,
    ...
    assert({
      given: "a downloaded (HTTP/HTTPS) scaffold",
      should: "return cleanupTip 'npx aidd scaffold-cleanup'",
      actual: result.cleanupTip,
      expected: "npx aidd scaffold-cleanup",
    });
  });

  test("cleanupTip is stable regardless of folder path containing spaces", async () => {
    const spacedProjectDir = path.join(tempDir, "my project with spaces");
    const result = await runCreate({
      folder: spacedProjectDir,
      resolveExtensionFn: fixtureResolve({ downloaded: true }),
    });
    ...
    assert({
      ...
      should:
        "still return the simple cleanup tip without embedding the folder path",
      actual: result.cleanupTip,
      expected: "npx aidd scaffold-cleanup",
    });
  });
