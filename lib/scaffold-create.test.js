...
describe("resolveCreateArgs", () => {
  ...
  test("two-arg: https:// URL as type with explicit folder is accepted", () => {
    const result = resolveCreateArgs(
...
describe("runCreate", () => {
  ...
  test("cleanup tip does not contain a relative folder name", async () => {
    ...
    assert({
      ...
      actual: path.isAbsolute(
        result.cleanupTip
          .replace("npx aidd scaffold-cleanup ", "")
          .replace(/^"|"$/g, ""),
      ),
      expected: true,
    });
  ...
  test("cleanup tip wraps a folder path containing spaces in double quotes", async () => {
    const remoteResolve = async () => ({
      extensionJsPath: null,
      manifestPath: "/fake/SCAFFOLD-MANIFEST.yml",
      readmePath: "/fake/README.md",
    ...
    assert({
      given: "a folder path that contains spaces",
      should: "wrap the path in double quotes in the cleanup tip",
      actual: result.cleanupTip,
      expected: 'npx aidd scaffold-cleanup "/home/user/my project"',
    });
  });
