...
describe("runCreate", () => {
  ...
  test("given --prompt and no --agentConfig, calls resolveAgentConfigFn with value:undefined so the env var and config file are consulted", async () => {
    const resolveCalls =
      /** @type {Array<{value?: string | undefined, cwd?: string}>} */ ([]);
    const trackingResolveAgentConfig = async (
      /** @type {{value?: string, cwd?: string}} */ { value, cwd },
    ) => {
      resolveCalls.push({ value, cwd });
    ...
    assert({
      given: "agentConfig is not passed to runCreate",
      should:
        "call resolveAgentConfigFn with value:undefined so AIDD_AGENT_CONFIG and aidd-custom/config.yml are consulted",
      actual: resolveCalls[0]?.value,
      expected: undefined,
    });
  });
