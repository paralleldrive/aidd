...
import { runCreate } from "./scaffold-create.js";
...
describe("runCreate — named scaffold integration (no local clone)", () => {
  /** @type {string} */
  let tempDir;
  /** @type {string} */
  let projectDir;
  ...
  beforeEach(async () => {
    tempDir = path.join(
      os.tmpdir(),
      `scaffold-create-named-test-${Date.now()}`,
    );
    await fs.ensureDir(tempDir);
    projectDir = path.join(tempDir, "my-project");
  });
  ...
  test("default scaffold (next-shadcn) resolves from npm package and copies files without a local clone", async () => {
    const { resolveExtension } = await import("./scaffold-resolver.js");
    const originalEnv = process.env.AIDD_CUSTOM_CREATE_URI;
    delete process.env.AIDD_CUSTOM_CREATE_URI;

    try {
      await runCreate({
        // no type — resolves to next-shadcn via DEFAULT_SCAFFOLD_TYPE
        folder: projectDir,
        // __dirname is lib/ — the same relative position as in the installed npm
        // package, where lib/ sits one level above ai/scaffolds/. This simulates
        // npx aidd create my-project running from the npm cache with no local clone.
        packageRoot: __dirname,
        resolveExtensionFn: (/** @type {any} */ opts) =>
          resolveExtension({
            ...opts,
            log: () => {},
            readConfigFn: async () => ({}),
    ...
    assert({
      given:
        "npx aidd create my-project with no type arg (no local clone of aidd repo)",
      should:
        "copy SCAFFOLD-MANIFEST.yml from the npm-bundled next-shadcn scaffold into the project folder",
      actual: await fs.pathExists(
        path.join(projectDir, "SCAFFOLD-MANIFEST.yml"),
      ),
      expected: true,
  ...
  test("named scaffold (next-shadcn) files are copied from npm package via fs.copy — same mechanism as URL scaffolds", async () => {
    const { resolveExtension } = await import("./scaffold-resolver.js");

    await runCreate({
      type: "next-shadcn",
      folder: projectDir,
      // __dirname is lib/ — same relative position as the installed npm package.
      // resolveNamed resolves path.resolve(packageRoot, '../ai/scaffolds/next-shadcn').
      packageRoot: __dirname,
      resolveExtensionFn: (/** @type {any} */ opts) =>
        resolveExtension({ ...opts, log: () => {} }),
      runManifestFn: async () => {},
    });

    assert({
      given:
        "npx aidd create next-shadcn my-project (named scaffold, no local clone)",
      should:
        "copy scaffold files from the npm-bundled package into the project folder via fs.copy (same mechanism as URL scaffolds)",
      actual: await fs.pathExists(
        path.join(projectDir, "SCAFFOLD-MANIFEST.yml"),
      ),
      expected: true,
