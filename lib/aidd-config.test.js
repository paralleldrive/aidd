...
describe("readConfig", () => {
  ...
  test("returns empty object when config file contains a bare string", async () => {
    const tempDir = await createTempDir();
    const configFile = path.join(tempDir, "config.yml");
    await fs.writeFile(configFile, "hello", "utf-8");

    const result = await readConfig({ configFile });

    assert({
      given: "a config.yml containing a bare string scalar",
      should: "return an empty object",
      actual: result,
      expected: {},
    });
  ...
  test("returns empty object when config file contains a YAML array", async () => {
    const tempDir = await createTempDir();
    const configFile = path.join(tempDir, "config.yml");
    await fs.writeFile(configFile, "- a\n- b\n", "utf-8");

    const result = await readConfig({ configFile });

    assert({
      given: "a config.yml containing a YAML array",
      should: "return an empty object",
      actual: result,
      expected: {},
    });
  ...
  test("returns empty object when config file contains a bare number", async () => {
    const tempDir = await createTempDir();
    const configFile = path.join(tempDir, "config.yml");
    await fs.writeFile(configFile, "42", "utf-8");

    const result = await readConfig({ configFile });

    assert({
      given: "a config.yml containing a bare number scalar",
      should: "return an empty object",
      actual: result,
      expected: {},
    });
...
describe("writeConfig", () => {
  test("creates config file with given updates as YAML", async () => {
    ...
    const parsed = /** @type {Record<string, unknown>} */ (yaml.load(content));
