...
const generateIndexContent = async (dirPath) => {
  ...
  const allSubdirs = await getSubdirectories(dirPath);

  // Filter subdirectories to only include those with indexable content
  const subdirs = [];
  for (const subdir of allSubdirs) {
    const subdirPath = path.join(dirPath, subdir);
    if (await hasIndexableContent(subdirPath)) {
      subdirs.push(subdir);
    }
  }
...
/**
 * Check if a directory has any indexable content (markdown files or subdirectories with content).
 * Recursively checks subdirectories to ensure they actually contain markdown files.
 */
const hasIndexableContent = async (dirPath, visited = new Set()) => {
  ...
  // If there are markdown files, we have content
  if (files.length > 0) {
    return true;
  }

  // Check if any subdirectory has indexable content (recursive)
  ...
  for (const subdir of subdirs) {
    ...
    if (!stats.isSymbolicLink()) {
      if (await hasIndexableContent(subdirPath, visited)) {
        return true;
      }
    }
  }
 ...
 * Returns null if directory has no indexable content (only non-markdown files)
 */
const writeIndex = async (dirPath) => {
  // Skip directories with no markdown files or subdirectories
  const hasContent = await hasIndexableContent(dirPath);
  if (!hasContent) {
    return null;
  }
...
const generateIndexRecursive = async (dirPath, results = [], depth = 0) => {
  ...
  // Generate index for current directory (returns null if no indexable content)
