...
// Directories where index.md is managed by other tools (e.g., skills-extractor)
const SKIP_INDEX_DIRS = ["commands"];

/**
 * Write index.md to a directory
 * Skips directories in SKIP_INDEX_DIRS (managed by other generators)
 */
const writeIndex = async (dirPath) => {
  const dirName = path.basename(dirPath);

  // Skip directories managed by other generators
  if (SKIP_INDEX_DIRS.includes(dirName)) {
    return {
      path: path.join(dirPath, "index.md"),
      success: true,
      skipped: true,
    };
  }
...
const generateIndexRecursive = async (dirPath, results = [], depth = 0) => {
  ...
  // Only include non-skipped results
  if (!result.skipped) {
    results.push(result);
  }
