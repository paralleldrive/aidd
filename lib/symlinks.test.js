...
describe("createSymlink error dispatch", () => {
  ...
  test("existing target without force throws with ValidationError cause", async () => {
    ...
    assert({
      ...
      should: "throw with ValidationError cause for handleCliErrors dispatch",
      actual: {
        name: caughtError?.cause?.name,
        code: caughtError?.cause?.code,
      },
      expected: { name: "ValidationError", code: "VALIDATION_ERROR" },
    });
  });
});
