...
describe("createSymlink error dispatch", () => {
  ...
  test("existing symlink target without force should set cause.name to ValidationError", async () => {
    ...
    try {
      ...
      assert({
        given: "a symlink target that already exists and force is false",
        should:
          "set error.cause.name to 'ValidationError' for handleCliErrors dispatch",
  ...
  test("existing symlink target without force should set cause.code to VALIDATION_ERROR", async () => {
    await fs.writeFile(path.join(tempTestDir, ".cursor"), "existing");
    ...
    try {
      ...
      assert({
        given: "a symlink target that already exists and force is false",
        should: "set error.cause.code to 'VALIDATION_ERROR'",
...
describe("cursor symlink functionality", () => {
  const tempTestDir = path.join(__dirname, "temp-symlinks-test");
