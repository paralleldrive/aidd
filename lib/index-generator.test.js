...
Content here.`;
    ...
    test("handles files without frontmatter", async () => {
      ...
      assert({
        ...
        should: "include the file title without a description placeholder",
        actual:
          content.includes("Plain File") &&
  ...
  describe("generateAllIndexes", () => {
    ...
    test("also indexes aidd-custom/ when it exists", async () => {
      ...
      assert({
        given: "ai/ and aidd-custom/ both exist",
        should: "include indexes for both directories",
        actual: result.indexes.length,
        expected: 2, // ai/ and aidd-custom/
      });
