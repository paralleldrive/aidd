...
describe("formatTable", () => {
  ...
  test("score display", () => {
    const output = formatTable([makeResult({ score: 20_940 })]);

    assert({
      given: "a score of 20940",
      should: "display it as a plain integer string without locale formatting",
      actual:
        output.includes("20940") &&
        !output.includes("20,940") &&
        !output.includes("20.940"),
      expected: true,
    });
  });
