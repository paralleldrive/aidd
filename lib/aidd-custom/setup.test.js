...
const createTempDir = async () => {
  const dir = path.join(
    os.tmpdir(),
    `aidd-custom-setup-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe("createAiddCustomConfig", () => {
  ...
  test("config.yml contains commented agent-config example", async () => {
    const tempDir = await createTempDir();
    await createAiddCustomConfig({ targetBase: tempDir })();
    const content = await fs.readFile(
      path.join(tempDir, "aidd-custom", "config.yml"),
    ...
    assert({
      given: "a newly created config.yml",
      should: "include a commented agent-config example line",
      actual: content.includes("# agent-config: claude"),
      expected: true,
    });
  });
