...
const CloneError = { code: "CLONE_ERROR", name: "CloneError" };
const FileSystemError = { code: "FILESYSTEM_ERROR", name: "FileSystemError" };
const ValidationError = { code: "VALIDATION_ERROR", name: "ValidationError" };
...
const executeClone = async ({
  ...
  try {
    ...
    // Create aidd-custom/config.yml with project defaults.
    // Never force-overwrite: config.yml holds user preferences, not framework code.
    verbose && logger.info("Creating aidd-custom/config.yml...");
    const customConfigResult = await createAiddCustomConfig({
      targetBase: paths.targetBase,
    })();
    verbose && logger.verbose(`aidd-custom: ${customConfigResult.message}`);

    // Generate aidd-custom/index.md so agents can read it immediately after install,
    // without waiting for the pre-commit hook to run.
    verbose && logger.info("Generating aidd-custom/index.md...");
    const customDir = path.join(paths.targetBase, "aidd-custom");
