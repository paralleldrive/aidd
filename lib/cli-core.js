...
// Windows file locking retry helper
// Windows can hold file handles longer than Unix, causing EBUSY errors
// Retry with exponential backoff to handle transient locks
const removeWithRetry = async (
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isRetryableError = error.code === "EBUSY" || error.code === "EPERM";
  ...
  async () => {
    ...
    try {
      ...
      if (cursorExists) {
        ...
        // Windows file locking: retry removal with delay
        await removeWithRetry(cursorPath, { maxRetries: 3, delayMs: 100 });
      }

      // Create symlink with Windows compatibility
      // 'junction' type works on Windows without admin privileges
      // and is compatible with directory targets
      const symlinkType = process.platform === "win32" ? "junction" : "dir";
      await fs.symlink(aiRelativePath, cursorPath, symlinkType);

      // Verify symlink was created successfully
      const created = await fs.pathExists(cursorPath);
      if (!created) {
        throw new Error(
          "Symlink creation appeared to succeed but file does not exist",
        );
      }
    } catch (originalError) {
...
const executeClone = async ({
  ...
  try {
    ...
    if (force && (await fs.pathExists(paths.target))) {
      await removeWithRetry(paths.target, { maxRetries: 5, delayMs: 150 });
    }
